...
class wpCSL_settings__mpcafe {
    ...
    function __construct($params) {
        ...
        $this->save_text =__('Save Changes',WPCSL__mpcafe__VERSION);
        $this->css_prefix = '';  
                            ...
                            '<p>Here are the technical details about your plugin:<br />
                               <div style="border: solid 1px #E0E0E0; padding: 6px; margin: 6px;
                                 ...
                                 <div style="clear:left;">
                                   ...
                                   <div style="float: left;">' . WPCSL__mpcafe__VERSION . '
                                   </div>
                                 </div>
                                 <div style="clear:left;">
                                   <div style="width:150px; float:left; text-align: right;
    ...
    function add_section($params) {
        if (!isset($this->sections[$params['name']])) {
            $this->sections[$params['name']] = new wpCSL_settings_section__mpcafe(
                array_merge(
                    $params,
        ...
        if (!isset($this->sections[$section])) {
            if (isset($this->notifications)) {
                $this->notifications->add_notice(
                    3,
                    sprintf(
                       __('Program Error: section <em>%s</em> not defined.',WPCSL__mpcafe__VERSION),
                       $section
                       )
                );
            }            
...
/****************************************************************************
 **
 ** class: wpCSL_settings_section__mpcafe
 **
 **/
class wpCSL_settings_section__mpcafe {
    ...
    function add_item($params) {
        $this->items[] = new wpCSL_settings_item__mpcafe(
            array_merge(
                $params,
                array('plugin_url' => $this->plugin_url,
                      'css_prefix' => $this->css_prefix,
...
/****************************************************************************
 **
 ** class: wpCSL_settings_item__mpcafe
 **
